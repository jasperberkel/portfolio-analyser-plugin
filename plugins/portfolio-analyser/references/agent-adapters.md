...
Researchers get only the inputs materialized for their task in the root workflow.json. The market worker must never inherit portfolio data. Strategy gets the complete prepared context including all included reports. Both can write only their assigned artifacts by instruction; the orchestrator alone connects to the app and updates run state.

Native research MCP connectors are optional. Keep provider/tool instructions in research/connectors.md. During later connector setup grant the required research tools in the host profile; the current Claude profile has web tools only. Never grant app publication to research workers. Reading a reference does not grant tools or authentication.
